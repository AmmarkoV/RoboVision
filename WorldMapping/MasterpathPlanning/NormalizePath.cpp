...
TraceNode * GetANormalizedLineFromNodes(TraceNode * rawnodes,unsigned int rawnodes_size,unsigned int &str8nodes_size)
{
  ...
  TraceNode tmpnodes[rawnodes_size]; // IN WORST CASE WE WILL HAVE THE SAME NUMBER OF NODES AS RAW NODES
  ...
  for (unsigned int i=0; i < rawnodes_size; i++ )
    {
  ...
  TraceNode * str8nodes = new TraceNode[str8nodes_size+1];
  if ( str8nodes == 0 )
    {
    ...
    {
      for (unsigned int i=0; i<str8nodes_size; i++)
        {
...
void GetTheShortestNormalizedLineFromNodes(NodeData * world,NodeNeighborsCount *world_neighbors,unsigned int world_x,unsigned int world_size,TraceNode * str8nodes,unsigned int &str8nodes_size)
{
  ...
  for (unsigned int i=str8nodes_size-1; i > 0; i-- ) // To i<=0 Dn exei noima giati dn mporw na zipparw ton prwto komvo , prepei na yparxei !!
    {
  ...
  if ( str8nodes_size_start != str8nodes_size )
    {
       if ( debug_normalize_path ==1 )
       {
        ...
        for (int i=0; i <str8nodes_size_start; i++ )
         {
           printf("( %d ) %d,%d ",i,str8nodes[i].nodex,str8nodes[i].nodey);
         }
        printf("\n\n");
        for (int i=0; i <str8nodes_size; i++ )
         {
           printf("( %d ) %d,%d ",i,str8nodes[i].nodex,str8nodes[i].nodey);
         }
        }
