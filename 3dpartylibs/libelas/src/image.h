...
image<uchar> *loadPPM(const char *name) {
    FILE *pf=0;
    pf = fopen(name,"rb");
    ...
    if (pf!=0 )
    {
        ...
        if ( (z < 1) || ( d != 255 ) ) { fclose(pf); return 0; }
             ...
             char * image_mem = (char * ) malloc ( w * h * 3 * sizeof(char) ); 
             size_t rd = fread(image_mem,3, w*h, pf);

             register char *px =  (char*) image_mem;
             register char *limit =  px + (w * h * 3); 
             
       
             image<uchar> *im = new image<uchar>(w, h);   
             register char *opx =  (char*) im->data;
             register char  *r, *g , *b;
             unsigned int col_med = 0;
             ...
             while ( px < limit)
             {
               r = px++; g = px++; b = px++;
               col_med=  ( *r + *g + *b )/3;
               if ( col_med < 255 ) { *opx= (char)col_med ; }
                                           else
                                    { *opx= 255; }
	      ++opx;
             }
...
void savePPM(image<uchar> *im, const char *name) 
{
    FILE *fd=0;
    fd = fopen(name,"wb");
	...
	{ 
         ...
         char * image_mem = (char * ) malloc ( width * height * 3 * sizeof(char) ); 
         
         register char *opx =  (char*) image_mem;
         register char *limit =  opx + (width*height*3); 
              
         register char *px =  (char*) im->data;
         register char  *monochrome;
