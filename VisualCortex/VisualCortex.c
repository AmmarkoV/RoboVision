...
void VisCortx_GetHyperVisorStatus(unsigned int print_std,unsigned int print_file,char * output)
{
 if ( (print_std) && ( output != 0 ) )
 {
  ...
  sprintf(tmpline," FIRST DERIV. , AVERAGE %u , LAST %u , SAMPLES %u \n",GetAverageTimer(FIRST_DERIVATIVE_DELAY),GetLastTimer(FIRST_DERIVATIVE_DELAY),GetTimesTimerTimed(FIRST_DERIVATIVE_DELAY)); strcat(output,tmpline);
