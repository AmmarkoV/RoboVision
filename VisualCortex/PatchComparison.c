...
inline int ComparePatchesUsingHistogram(int hist_reg_left,int hist_reg_right,unsigned int *source_x1,unsigned int *source_y1,unsigned int *target_x1,unsigned int *target_y1)
{
  if (  settings[DEPTHMAP_IMPROVE_USING_HISTOGRAM] ==0 ) { return 0; }
    ...
    if (
        ( ABSDIFF(hist1.median_r,hist2.median_r) > settings[PATCH_HIST_THRESHOLD_R])||
        ( ABSDIFF(hist1.median_g,hist2.median_g) > settings[PATCH_HIST_THRESHOLD_G])||
        ( ABSDIFF(hist1.median_b,hist2.median_b) > settings[PATCH_HIST_THRESHOLD_B])
       )
          {
	...
	while (y<height)
	{
      ...
      while (image_px1<stopx1)
	  {
	    ...
	    sobel_mismatch=0;
		 ...
		 sobel_score+=ABSDIFF(*sobel_px1,*sobel_px2);
		 ...
		 // BIGER SCORE -> MORE PATCH DIFFERENCE  !
		 second_deriv_score+=ABSDIFF(*secondderiv_px1,*secondderiv_px2);
        ...
        // ************** RGB COMPARISON **************
        // BIGER SCORE -> MORE PATCH DIFFERENCE  !
		rgb_score+= ( ABSDIFF(*image_px1,*image_px2)  );
		image_px1++; image_px2++;
		rgb_score+= ( ABSDIFF(*image_px1,*image_px2) );
		image_px1++; image_px2++;
		rgb_score+= ( ABSDIFF(*image_px1,*image_px2) );
        ...
        // ************** RGB COMPARISON **************
         ...
         // ************** MOVEMENT COMPARISON **************
		 // BIGER SCORE -> MORE PATCH DIFFERENCE  !
		 move_score+= ABSDIFF(*sobel_px1,*sobel_px2);
                                  ...
                                  )
{
    ...
    rgb_score= ABSDIFF(hist1.median_r,hist2.median_r)*patch_size_div_3;
    rgb_score+=ABSDIFF(hist1.median_g,hist2.median_g)*patch_size_div_3;
    rgb_score+=ABSDIFF(hist1.median_b,hist2.median_b)*patch_size_div_3;
    total_score= rgb_score * settings[DEPTHMAP_RGB_MULTIPLIER];
    /* ------------------------------------------------------------------------------------------------------ */
