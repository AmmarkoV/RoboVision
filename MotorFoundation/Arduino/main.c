...
{
    if (!kickstart_arduino_thread(devname) )
      {
    ...
    unsigned int tries=0 , max_tries = 1000;
    while ( (!RoboVisionSensorsOK())&&(tries<max_tries) ) { fprintf(stderr,"."); usleep(10000); ++tries; } fprintf(stderr,"\n\n");
